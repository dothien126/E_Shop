...
router.delete('/:id', (req, res) => {
    Order.findByIdAndRemove(req.params.id).then( async order => {
        if(order) {
            ...
            return res.status(404).json({success: false, message: 'the order is not created'})
        }
    }).catch(err => {
...
router.post(`/`, async (req, res) => {
    ...
    const orderItemsIdsResolved = await oderItemsIds

    const totalPrices = await Promise.all(orderItemsIdsResolved.map( async (orderItemsIds) => {
        const orderItem = await OrderItem.findById(orderItem).populate('product', 'price')
